...
- H2O single-molecule stability MD (Sputnik north-star): `examples/experiments/h2o_molecule_stability.js`
...
- Sputnik single-molecule stability landed (`examples/experiments/h2o_molecule_stability.js`): the north-star "stable H2O molecule without exploding" item, as a classical flexible-water MD (harmonic O-H bonds + H-O-H angle, velocity-Verlet NVE) run in the deterministic hook layer (`threads: 1`, no RNG → byte-reproducible). Over 400 fs the bonds ring around equilibrium (mean 0.958 Å, max 1.157 Å), angle stays at 104.5°, total energy drifts <0.5%. The `h2o_molecule_bonds_stable` validation case (category `molecule`) guards it (suite 23 cases, 19 pass / 0 fail). Honest scope: Geant4 transports particles but cannot form/evolve bound molecular states, so the bonds are classical MD with Geant4 as the per-tick clock.

...
- Sputnik bulk-water scale-up landed (2026-06-11): `h2o_bulk_water.js` N 48 → 108 molecules (box 14.8 Å), DSF cutoff 5.4 → 7.0 Å, RDF_BINS 150 over a 7.4 Å range — the **~4.5 Å tetrahedral second shell is resolved** (4.41 Å, height 1.10; `second_shell_near_tetrahedral` flag, reported not folded into `bulk_water_stable`), first peak 2.785 Å, mean T now production-phase-only 302.9 K. Coordination is dual-reported (`coordination_number` to the model's own shallow first min ≈7.4; `coordination_number_to_3p4A` at the experimental convention ≈4.9 vs ~4.3-4.7 measured) because the model's inter-shell depletion is too shallow/far (min g≈0.95 @3.82 Å vs ~0.75 @~3.4 Å) — stated, not tuned. Run cost ~4.3 min (O(N²) pair loop in QuickJS), still gated by `SKIP_BULK`. Beam-profile presets also landed: `helpers.beamProfiles.spread(name, overrides)` (`pencil`/`laser`/`ledLamp`/`flashlight`/`sunbeam`); `glass_of_water_varied.js` adopted `ledLamp` (its historical values — verified hash-preserving, canonical beam JSON byte-identical).

...
- Sputnik bulk-water SPC/E parameterisation landed (2026-06-11): moved the force field from plain-SPC to **flexible SPC/E** (Berendsen 1987: q_O=-0.8476/q_H=0.4238, r_OH=1.0 Å, HOH 109.47°, σ=3.166 Å, ε=0.1553 kcal/mol; flexible harmonic intramolecular terms about that geometry, not rigid constraints). The larger charges + tetrahedral angle deepened the inter-shell depletion as intended: O-O g(r) first minimum 0.95 → **0.85** (exp ~0.75) at 3.82 → **3.18 Å** (exp ~3.4), fixing the coordination over-count — **7.36 → 3.78** (to own min) and **4.89 → 4.70** (to the 3.4 Å convention), both now in the measured ~4.3-4.7 band; first peak 2.785 → 2.736 Å (still in [2.6,3.0]), second shell 4.41 → 4.26 Å. New emitted observable `gr_first_min_height` (depletion depth) surfaced in the validation case + video end card; nothing tuned to a target. Honest residual: depletion still 0.85 vs 0.75 (short-cutoff DSF), the next lever being full Ewald / bigger box.

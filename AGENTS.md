...
- CNT electronic-structure experiment (Vostok): `examples/experiments/cnt_band_structure.js` (tight-binding zone-folding band gaps / metallicity vs chirality); render `tools/viz/demos/render_cnt_band_structure.py` → `cnt_band_structure.png`
...
- Vostok (CNT) **electronic structure** landed (2026-06-12): `examples/experiments/cnt_band_structure.js` adds the ROADMAP's "electron behaviour differences ... Fermi gap modelling" — a hook-layer **tight-binding zone-folding** model computing, per (n,m) chirality, the diameter, chiral angle, metallic/semiconducting character and band gap, compared to two textbook+measured results: the metallicity rule (metallic iff (n-m) mod 3 == 0) and the gap law (semiconducting E_g = 2·a_cc·γ0/d, so E_g·d ≈ 0.82 eV·nm, measured 0.7-0.9). Over a 26-tube panel: 12 metallic / 14 semiconducting all classified correctly, E_g·d constant at 0.8236 eV·nm (γ0=2.9 eV, a_cc=0.142 nm), and STM anchors (10,0)→1.05 / (13,0)→0.80 / (17,0)→0.62 eV matched within 1.2%. Honest scope (same as the H2O MD): Geant4 transports electrons through a representative (10,0) tube geometry but does NOT compute band structure; the electronic structure is the hook-layer "physics for comparison". Leading-order zone-folding — curvature secondary gaps in nominally-metallic tubes and the trigonal-warping family (Kataura) split are stated residuals/next steps. Fast (no MD; 5 electron events). Guarded by the `cnt_band_structure` validation case (category `cnt`), rendered as `cnt_band_structure.png`. Added `constants.carbonBondLengthNm` (0.142) to `trech_helpers.js`.

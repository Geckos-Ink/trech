...
- Beam source variety draws from Geant4's seeded engine: `beam.originMm` + `beam.spread` (`spotRadiusMm`/`divergenceDeg`/`energySpreadFractional`) spread position/direction/energy, and `beam.polarization` (`""`/`unpolarized` → random transverse linear state per event; `linear` + `polarizationAngleDeg` → fixed; `none` → legacy Geant4 fallback) controls optical-photon polarization. Polarization is applied only to optical photons (other particles untouched), kills the `ZeroPolarization` warning by default, and is conditionally serialized so existing config hashes hold. `TrechPrimaryGeneratorAction` owns all of this; keep it reproducible under a fixed seed.

...
- `examples/experiments/polyurethane_foam.js`: **the polyurethane foam experiment ("the solid sponge") in an open cup** — two viscous liquids are poured together (Solution A: polyol + a little water; Solution B: a diisocyanate) and within seconds the mixture creams, expands toward ~30× its volume, and cures into a **rigid porous sponge**. TRECH is told only what is *in the cup*: the two-part recipe (hydroxyl number, water pphp, isocyanate index, catalyst/surfactant levels) and the Geant4-constructed solution materials. Geant4 reports the **isocyanate nitrogen (8.4e21 /cm³ in Solution B)**, the A/B density contrast (1.08 vs 1.22 g/cm³), and the mixed liquid's derived colour; a two-stage `ctx.cascade` (nano reagent descriptors → macro observer-band response) infers the **coefficients of a reduced dual-reaction foaming model** — gel (`R−N=C=O + R'−OH → urethane`) and blow (`R−N=C=O + H₂O → amine + CO₂↑`) rates, Arrhenius activation, per-reaction exotherms, CO₂ capacity/saturation, Flory gel point, Castro-Macosko viscosity exponent, surfactant trapping, expansion mobility. The cascade emits **no** expansion ratio, milestone time, colour, or final consistency: a deterministic hook-layer integrator advances the two coupled conversions and **the foam itself emerges** — an 18 s dissolution induction then cream, rise to **26.9×** (96% gas), gel at 65 s, a +54 K exotherm, **93% of the CO₂ trapped by the rising viscosity**, and a rigid sponge whose parcel motion freezes past the gel point (all graded against known PU-foam behaviour only at run end). PubChem contributes structure identity only (glycerol/TDI/water CID+SMILES+formula, element-set cross-checked against the Geant4 composition). Emits 141 `material_frame`s (persistent parcels + metaball `render_surface`) + `polyurethane_foam_summary`; rendered by classic `trech-viz` → `polyurethane_foam.gif`; guarded by `polyurethane_foam_expansion` (**15 checks**). Honest scope: Geant4 does not solve urethane kinetics or bubble rheology — the reduced foaming model is a labelled "physics for comparison" hook-layer model whose coefficients are inferred from the Geant4 base; the compact macro response surface is illustrative (σ=0.14 emitted), and the cream/tan swatches are labelled representation while the whitening/expansion *timing* is the emergent, graded result. A wider trained reactive-foam panel is tracked in `ROADMAP.md`.
...
| [`polyurethane_foam.js`](examples/experiments/polyurethane_foam.js) | **polyurethane foam ("the solid sponge")**: cream→rise→gel→solid emerges from the two-part recipe alone — 26.9× expansion (96% gas), +54 K exotherm, 93% of the CO₂ trapped by the rising viscosity, then a rigid sponge with motion frozen | Geant4-built solution materials report the isocyanate N + A/B density contrast + liquid colour; a two-stage cascade infers the dual-reaction (gel + blow) foaming coefficients — expansion/timing/rigidity are emergent, not typed | `polyurethane_foam_expansion` |
...
<tr>
<td align="center"><img src="https://github.com/Geckos-Ink/trech/blob/main/tools/viz/demos/polyurethane_foam.gif?raw=true" width="300"/><br/><sub><b>polyurethane_foam</b> — the solid-sponge experiment in classic TRECH 3D: two liquids (polyol+water / diisocyanate) cream after an 18 s induction, rise to <b>26.9×</b> as the water–isocyanate reaction blows CO₂ into the curing urethane matrix, and freeze into a rigid porous sponge past the gel point — expansion, exotherm, milestone ordering, and the rigid endpoint all <i>emerge</i> from the Geant4 material base + cascade-inferred coefficients</sub></td>
...
- Lava lamp + shared Studio/classic 3D playback (corrected 2026-07-16):
  ...
  topology, retained fine parcel lineage plus continuous fluid-interface necking, non-axis-locked 3D transport, and fixed-inventory spatial/temporal refinement convergence. The current validation corpus reports **44 cases, 40 pass / 0 fail-error / 0 skip /

...
- `examples/experiments/lava_lamp_10_minutes.js`: **a lava lamp running for exactly ten physical minutes.** Geant4 `G4_WATER`/`G4_PARAFFIN` material probes and derived optics seed a two-stage `ctx.cascade`; its period (108.8 s), vertical excursion, cohesion, phase heterogeneity, and σ=0.12 drive a deterministic observer replay. The run emits 61 held `material_frame`s with 900 conserved wax representatives on physical 0–600 s and explicit 0–6 s / 100× playback clocks. Studio and classic `trech-viz` render the same scene/frames ([Studio GIF](studio/tests/reference/lava_lamp.gif), [classic 3D GIF](tools/viz/demos/lava_lamp_trech_viz.gif)); the README GIFs are readable 10-second captures of the first retained physical minute, while the run and validation remain ten minutes. Orange/blue/housing colours are labelled display choices. Guarded by `lava_lamp_ten_minutes` (8 checks). Geant4 does not solve phase change, heat flow, or convection: the response surface and blob kinematics remain illustrative, with wider measured training coverage tracked in `ROADMAP.md`.
...
- Lava lamp + shared Studio/classic 3D playback (2026-07-16): `lava_lamp_ten_minutes` passes
  **8/8** checks—Geant4 material base, two-stage cascade drive, exactly 600 s / 10 min, declared
  ...
  the same 61 emitted frames; the latter viewer now honours rotations, `viz_*` hints, RGBA, and
  observer clocks. The illustrative σ=0.12 macro response is not presented as a calibrated

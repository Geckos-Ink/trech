...
*The thesis in miniature: a biological cell rids itself of a small lipophilic **waste** molecule by passive permeation across its lipid bilayer (Overton's rule), while keeping its polar **essential** molecules — coarse-grained MD in the deterministic hook layer (one Geant4 event = one tick). **Right panel:** the simulated internal count N(t) (green) vs the classical **first-order clearance law** N₀·e^(−kt) (amber, Fick's law) — random microscopic permeation events reproduce the macroscopic kinetics (R² ≈ 0.985, half-life ~1226 ticks). The per-encounter permeation probability is scaled by a **Geant4-derived** membrane-vs-cytosol EM interaction ratio (G4EmCalculator, emitted live as `analytic_checks`; the mapping is illustrative and flagged as such) — nanoscale physics → mesoscale simulation → closed-form comparison. Render with `tools/viz/demos/render_efflux.py`.*
...
- `examples/experiments/testscenario_efflux.js`: **headline biological comparison** — a cell clearing a lipophilic waste molecule by passive lipid-bilayer permeation (Overton's rule) into an extracellular sink, while retaining polar essentials. The coarse-grained hook MD reproduces the classical first-order clearance law `N(t)=N₀·e^(−kt)` (Fick), validated by a log-linear fit (`R²≈0.985`); the per-encounter permeation probability is scaled by a Geant4-derived membrane/cytosol EM interaction ratio (`analytic_checks` via `G4EmCalculator`, illustrative). Emits `efflux_snapshot`/`efflux_summary` for `tools/viz/demos/render_efflux.py` (TRECH-vs-law comparison video). Guarded by the `efflux_first_order_kinetics` validation case.
...
- Membrane-efflux comparison scenario landed (`examples/experiments/testscenario_efflux.js`): a cell clears a lipophilic waste molecule by passive lipid permeation (Overton's rule), reproducing the classical first-order clearance law `N(t)=N₀·e^(−kt)` (log-linear fit R²≈0.985, half-life ~1226 ticks, 78/80 cleared, 30/30 essentials retained). The per-encounter permeation probability is scaled by a Geant4-derived membrane/cytosol EM interaction ratio (`G4EmCalculator`, μ_lipid≈0.0291/mm vs μ_water≈0.0377/mm at 30 keV → ratio 1.30, emitted live as `analytic_checks`; illustrative mapping). Rendered by `tools/viz/demos/render_efflux.py` → `tools/viz/demos/efflux_clearance.mp4`/`.gif`. Guarded by the `efflux_first_order_kinetics` validation case (suite now 29 cases, 25 pass / 0 fail / 4 info); `ctest --preset dev` 9/9.

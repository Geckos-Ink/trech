...
| [`polyurethane_foam.js`](examples/experiments/polyurethane_foam.js) | **polyurethane foam ("the solid sponge")**: cream→rise→gel→solid from the two-part recipe alone, plus the **gravity consequences** — the bun leans under its own weight, cracks, sheds pieces, and the pieces land on the table; a zero-g control removes all of it | Geant4-built solution materials report the isocyanate N + A/B density contrast + liquid colour; a two-stage cascade infers the dual-reaction chemistry **and** the foam mechanics, then a bonded-parcel network under standard gravity is integrated — expansion, timing, lean, cracks and debris are emergent, not typed | `polyurethane_foam_expansion` |

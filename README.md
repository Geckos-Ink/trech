...
- `examples/experiments/lava_lamp_10_minutes.js`: **a parameterized lava lamp whose validated default runs for exactly ten physical minutes.** Geant4 `G4_WATER`/`G4_PARAFFIN` probes and derived optics seed a two-stage `ctx.cascade`; its period (108.8 s), vertical excursion, cohesion, phase heterogeneity, and σ=0.12 drive the observer replay. Typed Studio options control physical duration, playback duration, and real Geant4 tick count. The default remains 600 s / 60 ticks / 61 frames. The [Studio GIF](studio/tests/reference/lava_lamp.gif) and [classic 3D GIF](tools/viz/demos/lava_lamp_trech_viz.gif) instead come from a separate 60 s / 100-tick run: 101 distinct simulated states at 0.6 s spacing feed 100 post-tick GIF frames over exactly ten seconds—no slowed seven-frame excerpt, optical flow, or temporal interpolation. Guarded by `lava_lamp_ten_minutes` (10 checks, including frame-per-tick correspondence and the README run's 101 unique states). Orange/blue/housing colours are labelled display choices. Geant4 does not solve phase change, heat flow, or convection: the response surface and blob kinematics remain illustrative, with wider measured training coverage tracked in `ROADMAP.md`.
...
| [`lava_lamp_10_minutes.js`](examples/experiments/lava_lamp_10_minutes.js) | 10-minute observer cycle: wax rises/cools/falls and visibly splits/merges; 61 held frames, 900 conserved representatives, 100× declared playback | Geant4 water/paraffin material+optics base auto-seeds a two-stage cascade; heat/convection replay is labelled illustrative | `lava_lamp_ten_minutes` |
...
- Lava lamp + shared Studio/classic 3D playback (2026-07-16): `lava_lamp_ten_minutes` passes
  **10/10** checks—Geant4 material base, two-stage cascade drive, one frame per Geant4 tick,
  the README run's 100 ticks / 101 unique states,
  exactly 600 s / 10 min, declared
  100× observer clock, bottom/top travel, rising/falling, split/merge, and 900/900 representative
  inventory. The current validation corpus reports **41 cases, 37 pass / 0 fail-error / 0 skip /
  4 info**. Studio's committed WGSL GIF and the classic PyVista GIF are two representations of
  the same output. The README run is separately simulated at 100 ticks/minute so its 100 GIF
  frames are fresh states rather than a slowed sparse timeline. The classic viewer honours
  rotations, `viz_*` hints, RGBA, and observer clocks. The illustrative σ=0.12 response is not a calibrated

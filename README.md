...
- Secondary reference ("Vostok" milestone): simulate carbon nanotube variants (structure, chirality, diameter) and electron behavior differences, including Fermi gap modeling, per `docs/CNT/BackToTheCarbon.md`. **First electronic-structure step landed:** `examples/experiments/cnt_band_structure.js` reproduces the metallic/semiconducting classification ((n−m) mod 3 rule) and the band-gap ∝ 1/diameter law on STM-measured anchors via tight-binding zone-folding (`cnt_band_structure.png`).

...
- `examples/experiments/lava_lamp.js`: **a duration-independent, stateful lava-lamp thermofluid scenario.** Geant4 probes water and a configured paraffin/density-modifier reference blend; the two-stage `ctx.cascade` infers melting, thermal expansion, heat transfer/diffusion, viscosity/drag, cohesion, interfacial velocity coupling, and carrier circulation. A bounded-step solver advances persistent parcel IDs through temperature, liquid fraction, density, buoyancy, inferred carrier advection, velocity coupling, boundaries, and neighbour topology—without a target cycle, authored trajectory, phase schedule, birth, or regeneration. Precision is no longer a single “more particles” knob: parcel count refines a fixed wax volume, `max_physics_step_s` controls time integration, `simulation_ticks` controls output sampling, and `render_surface_grid_mm` affects only the labelled display reconstruction. A 480-parcel/0.2 s validation converges against the 240-parcel/0.4 s run while preserving 4723.92 mm³ of wax. The [Studio GIF](studio/tests/reference/lava_lamp.gif) and [classic 3D GIF](tools/viz/demos/lava_lamp_trech_viz.gif) show the complete 600 s response in ten seconds as Gaussian-density 3D surfaces. The README run records 14 parcel-lineage coalescences, 16 fissions, and merged bodies in 58/101 states; both viewers reveal those emitted changes instead of manufacturing them. Emitted centres and clocks are unchanged; 100 post-tick states map directly to 100 GIF frames with no interpolation or optical flow. Guarded by `lava_lamp_inferred_thermofluid` (**21 checks**), including visible merge/split lineage, temporal coherence, duration/condition, and precision-refinement controls. Geant4 does not itself solve phase change or CFD; the compact response and parcel discretisation remain illustrative, with wider held-out training tracked in `ROADMAP.md`.
...
- Lava lamp + shared Studio/classic 3D playback (corrected 2026-07-16):
  `lava_lamp_inferred_thermofluid` passes **21/21** checks over a real Geant4 material base,
  ...
  topology, measured coalescence/fission lineage, and fixed-inventory spatial/temporal refinement convergence. The current validation corpus reports **41 cases, 37 pass / 0 fail-error / 0 skip /

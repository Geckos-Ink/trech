...
- `examples/experiments/lava_lamp.js`: **a duration-independent, stateful lava-lamp thermofluid scenario.** Geant4 probes water and a configured paraffin/density-modifier reference blend (explicitly a simulation proxy, not a product recipe); the two-stage `ctx.cascade` infers melting, thermal expansion, heat transfer/diffusion, viscosity/drag, and cohesion coefficients. A bounded-step solver then advances the same ordered 240 parcel IDs through temperature, liquid fraction, density, buoyancy, velocity, boundaries, and neighbour topology. There is no inferred cycle period, authored trajectory, phase schedule, particle birth, or regeneration. Duration, output cadence, resolution, heater, and ambient conditions are typed Studio inputs; 600 s is merely the default integration horizon. The [Studio GIF](studio/tests/reference/lava_lamp.gif) and [classic 3D GIF](tools/viz/demos/lava_lamp_trech_viz.gif) now show the complete 600 s default-condition response in ten display seconds: a dedicated 100-tick run emits 101 persistent states at 6 s spacing, and its 100 post-tick states map directly to the GIF frames with no interpolation or optical flow. Guarded by `lava_lamp_inferred_thermofluid` (17 checks): a 60 s horizon reproduces the default run at t=60 within 1e-9 mm, while an otherwise identical 310 K heater control stays essentially solid/dense and the 333.15 K run melts and crosses carrier density. Geant4 does not itself solve phase change or CFD; the compact response surface and parcel discretisation remain illustrative, with wider held-out training tracked in `ROADMAP.md`.
...
- Lava lamp + shared Studio/classic 3D playback (corrected 2026-07-16):
  `lava_lamp_inferred_thermofluid` passes **17/17** checks over a real Geant4 material base,
  ...
  topology. The current validation corpus reports **41 cases, 37 pass / 0 fail-error / 0 skip /
  ...
  600 s / 100-tick output, mapping its 100 post-tick states directly into ten display seconds

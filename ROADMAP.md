...
- **Validation report curation**: 22 cases now (17 at first commit — 12 pass, 4 info, 1 was wrong-spec and is now structural numeric replay — plus an h2o_fluid brine scenario guard, two fluid-physics scenario guards (Pascal's principle, osmosis), an end-to-end optics-surrogate transport guard, and an anti-degeneration sampling-diversity guard). Expand coverage as new outputs/scenarios land. Treat `docs/validation_report.md` as a regression artefact: re-generate via `scripts/run_validation_suite.sh` whenever the engine or scenarios change, and commit the regenerated report alongside the code change.

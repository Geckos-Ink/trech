...
- **Validation report curation**: 28 cases now (the analytic Beer-Lambert classical-vs-Geant4 cross-check is the latest; 17 at first commit — 12 pass, 4 info, 1 was wrong-spec and is now structural numeric replay — plus an h2o_fluid brine scenario guard, two fluid-physics scenario guards, an end-to-end optics-surrogate transport guard, an anti-degeneration sampling-diversity guard, and the three Sputnik molecular-scale guards: single-molecule bond stability, multi-molecule cluster fluid stability, and bulk-water g(r) structure). The osmosis guard was tightened on 2026-06-28: it now catches unbounded Brownian heating and requires dimensional exclusion, net water shift, early crossings, macroscopic flux growth, bounded thermostat energy, and late pressure bias (`net_water_flux_out=71`, max KE 0.961 vs target 0.81, external/internal pressure 1.46). The scenario also emits `osmotic_particles` for `tools/viz/demos/render_osmotic.py`, a 3D replay video driven by TRECH hook state rather than fixed formulas. Expand coverage as new outputs/scenarios land. Treat `docs/validation_report.md` as a regression artefact: re-generate via `scripts/run_validation_suite.sh` whenever the engine or scenarios change, and commit the regenerated report alongside the code change.
...
- Osmotic 3D replay video landed (2026-06-28): `testscenario_osmotic.js` now emits rounded `osmotic_particles` snapshots every 50 ticks as a deterministic viz/training sideband, and `tools/viz/demos/render_osmotic.py` renders `osmotic_dehydration.mp4` from those emitted states plus water trails, pore flow glyphs, and count history. The animation is a TRECH-output replay, not a closed-form osmotic-law drawing; larger-scale surrogates should train/gate on these Geant4-driven outputs. Honest next step: implement a stable spring/mesh membrane in the scenario so crenation/shrink is an emitted physical state, not a renderer-only effect.

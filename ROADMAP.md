...
## 2026-07-16 ten-minute lava lamp and shared 3D playback checkpoint

- **[landed] Observer-scale lava-lamp scenario.**
  `examples/experiments/lava_lamp_10_minutes.js` runs 60 Geant4 geantino clock ticks and emits
  61 held `material_frame`s spanning exactly 0–600 physical seconds beside an explicit 0–6 s /
  100× observer clock. Geant4 `G4_WATER`/`G4_PARAFFIN` probes plus derived optics auto-seed a
  two-stage `ctx.cascade` (`data/lava_lamp_cascade/`); its inferred 108.8 s period, vertical
  excursion, cohesion, and phase heterogeneity drive 900 conserved wax representatives through
  rise/cool/fall/split/merge motion. `lava_lamp_ten_minutes` guards 10/10 internal/contract
  checks, including one emitted observer frame per Geant4 tick and the dense README run.
  Honest scope: Geant4 does not solve phase change, heat transfer, or convection; the portable
  macro response surface and deterministic blob kinematics are illustrative and emit σ=0.12.
...
- **[corrected 2026-07-16] Documentation cadence comes from simulation, not slow playback.** The
  first one-minute GIF revision stretched seven frames from the default sparse ten-minute run,
  which made Studio appear unable to update dynamically. The scenario now exposes typed physical
  duration, playback duration, and Geant4 tick count. README generation performs a distinct
  60 s / 100-tick run, emitting 101 unique states at 0.6 s intervals; both renderers map post-tick
...
- **[open, concrete] Graduate the lava response beyond illustration.** Assemble a measured or
  ...
  quantitative cycle-period or thermal-fluid claim. Studio's soft sprites also remain subject to

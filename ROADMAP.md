...
3. **Coverage/uncertainty per band. [mechanism landed 2026-07-24]** Each stage now reports when it
   ...
   held-out R² surfaced). **Remaining:** *acting* on the flags — routing an out-of-domain / off-band
   event into the resim/exceptional-event path (still surfaced-not-acted); wiring the planner's
   starved-region signal (density inside the hull, not just the hull edge) into per-stage confidence;
   and per-band held-out accuracy only becomes *meaningful* once workstream 2 lands a real trained
   chain (the mechanism is ready, illustrative maps still report null).

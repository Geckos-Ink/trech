...
3. **Coverage/uncertainty per band. [mechanism landed 2026-07-24]** Each stage now reports when it
   ...
   `cascade_multiscale_demo.js` (emits per-stage coverage). **Remaining:** (a) feed the
   out-of-domain flag into the resim/exceptional-event path (currently surfaced, not yet acted on);
   (b) the harvester's dimension-scale band + the planner's starved-region signal are not yet wired
   into the per-stage confidence (a natural next tie-in); (c) per-band *held-out accuracy* reporting
   waits on workstream 2's real trained stages (a heuristic-domain illustrative map has no measured
   accuracy to report).

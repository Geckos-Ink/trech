...
- **[corrected] Replace the authored replay with persistent inferred state.** The prior
  ...
  density, buoyancy/drag, cohesion, neighbour topology, velocity, and boundaries. No cycle
  period, target excursion, phase schedule, trajectory, particle birth, or regeneration remains.
  `duration_s` is only the integration horizon; heater, ambient, output cadence, and parcel
  resolution are independent typed Studio inputs.
- **[landed] Falsifiable duration and condition controls.** `lava_lamp_inferred_thermofluid`
  guards 17/17 checks. A separate 60 s / 12-tick run has the same bounded internal steps as the
